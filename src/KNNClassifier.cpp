...
void KNNClassifier::addExample(float input[], int class_)
{
  // create new node
...
int KNNClassifier::classify(float input[], int k)
{
  if (k == 0 || k > getCount()) {
    _confidence = NAN;
  ...
  for (int i = 0; i < k; i++) {
    ...
    for (int j = 0; j < k; j++) {
      if (classCounts[j].class_ == INT_MIN) {
